...
- `cookies().set(...)` and object-form `cookies().set({ name, value, ... })` cookie storage.
- `cookies().delete(...)` logout cookie deletion evidence.
- `NextResponse.cookies.set(...)` and `NextResponse.cookies.delete(...)` cookie storage and logout evidence.
...
- Auth.js/NextAuth provider-managed session, JWT, refresh, callback, and logout behavior unless local application source exposes concrete token lifecycle calls.
...
- `res.cookie(...)` cookie storage and security attributes.
- `res.clearCookie(...)` logout cookie deletion evidence.
- Static `Set-Cookie` header writes through representative response header APIs.
...
- Passport or provider strategy behavior unless local source exposes concrete lifecycle calls. Provider/library adapters are tracked outside `#18`.
...
- `Response.set_cookie(...)` and `Response.delete_cookie(...)` storage and logout evidence.
...
- Provider-managed OAuth/OIDC behavior not represented in local source.
...
- Provider-managed social-auth or OAuth behavior unless local application source exposes concrete lifecycle calls.

## Verification expectations

Every supported framework family should have representative fixtures and tests that assert artifacts, lifecycle evidence, framework hints, conservative findings, and sanitized reports. Limitation fixtures or assertions should ensure provider-managed or opaque behavior does not produce overconfident findings.
